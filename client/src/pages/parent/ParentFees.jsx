...
function ReceiptModal({ loading, data, onClose }) {
  ...
  const downloadReceipt = () => {
    const lines = [
      ...
      '-----------------------------------------------',
      ...data.items.map((i) => `  ${i.name.padEnd(28)}  ₦${Number(i.amount).toLocaleString()}`),
      '-----------------------------------------------',
      `  Total        ₦${Number(data.total).toLocaleString()}`,
      `  Paid         ₦${Number(data.amountPaid).toLocaleString()}`,
          ...
          <table className="table">
            ...
            <tbody>
              {data.items.map((i, idx) => (
                <tr key={idx}><td>{i.name}</td><td className="right">{formatNaira(i.amount)}</td></tr>
              ))}
              <tr className="table__total"><td>Total</td><td className="right">{formatNaira(data.total)}</td></tr>
              <tr><td>Amount Paid</td><td className="right">{formatNaira(data.amountPaid)}</td></tr>
...
function buildReceiptHTML(r) {
  const items = r.items
    .map((i) => `<tr><td>${i.name}</td><td class="right">₦${Number(i.amount).toLocaleString()}</td></tr>`)
    .join('');
