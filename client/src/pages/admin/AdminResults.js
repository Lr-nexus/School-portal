...
function buildBatchReportHTML(batch) {
  const { className, session, term, cards } = batch;
  ...
  const oneCard = (card, index) => `
    <section class="report-card ${index === cards.length - 1 ? 'report-card--last' : ''}">
      ...
      <div class="rc-meta">
        Issued ${escapeHtml(card.issuedAt)} · Computer-generated document
      </div>
    </section>
  `;
...
<head>
<meta charset="utf-8" />
<title>Report Cards — ${escapeHtml(className)}</title>
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ---------- Print-only tightening ---------- */
  @media print {
    body { font-size: 11.5px; }
    .rc-head { padding-bottom: 10px; margin-bottom: 14px; }
    .rc-student { margin-bottom: 14px; }
    .rc-table th, .rc-table td { padding: 6px 8px; }
  }
</style>
</head>
<body>
  ${cards.map(oneCard).join('')}
</body>
</html>`;
}

/* Hidden-iframe print (no popup blocking) */
...
export default function AdminResults() {
  ...
  /* ==================================================================
     List view — pick a student OR print the whole class
     ================================================================== */
  return (
    <div>
      <PageHeader
...
function PrintClassModal({ classes, defaultClass, onClose, onDone }) {
  ...
  const [preview, setPreview] = useState(null); // { count, session, term }
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const html = buildBatchReportHTML(batch);
      printViaIframe(html);
      onDone();
    } catch (ex) {
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {preview && (
          <div className="alert alert--info" style={{ fontSize: 13 }}>
            ...
            <strong>{className}</strong> — {preview.session} · {preview.term}
          </div>
        )}

        <div className="alert alert--info" style={{ fontSize: 12, marginTop: 6 }}>
          💡 Each card prints on its own page. Recommended: pick <strong>Save as PDF</strong> in
          the print dialog to email the whole batch.
        </div>
